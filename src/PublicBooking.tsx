...
export function PublicBookingV2() {
  ...
  return <Shell><div className="booking"><div className="booking-intro"><p className="eyebrow">Agendamento online</p><h1>Seu próximo corte começa aqui.</h1><p>Escolha o serviço, o horário e venha se cuidar com a gente.</p><div className="steps">{['Serviço', 'Profissional', 'Horário', 'Seus dados'].map((label, index) => <span className={step === index + 1 ? 'current' : step > index + 1 ? 'completed' : ''} key={label}><i>{index + 1}</i>{label}</span>)}</div></div><div className="booking-card">
    ...
    {step === 2 && <><h2>Tem preferência de profissional?</h2><p>Você pode escolher alguém ou deixar a gente encontrar o melhor horário.</p><div className="pro-options"><button type="button" className={professionalId === 'qualquer' ? 'selected' : ''} onClick={() => setProfessionalId('qualquer')}><span className="avatar light">?</span><span><strong>Qualquer profissional disponível</strong><small>Mostra mais horários</small></span></button>{profissionais.map((pro) => <button type="button" className={professionalId === pro.id ? 'selected' : ''} onClick={() => setProfessionalId(pro.id)} key={pro.id}><span className="avatar" style={{ background: pro.cor }}>{pro.iniciais}</span><span><strong>{pro.nome}</strong><small>{pro.especialidades}</small></span></button>)}</div></>}
    {step === 3 && <><h2>Escolha um horário</h2>{bookingError && <p className="form-error">{bookingError}</p>}<button type="button" className="outline small" disabled={loadingSlots} onClick={() => setSlotVersion(v => v+1)}>Atualizar horários</button><div className="booking-date"><label>Dia<input type="date" value={date} min={today} onChange={(event) => { setDate(event.target.value); setTime('') }} /></label></div>{loadingSlots && <p className="empty">Buscando horários disponíveis…</p>}<div className="slots">{availableSlots.map((slot) => <button type="button" className={time === slot ? 'selected' : ''} key={slot} onClick={() => setTime(slot)}>{slot}</button>)}</div>{!loadingSlots && !availableSlots.length && <p className="empty">Não há horários disponíveis neste dia.</p>}</>}
