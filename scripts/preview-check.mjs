...
const server = await createServer({
  ...
  plugins: [{ name:'qa-local-context',enforce:'pre',
    transform(source,id) {
      if(id.endsWith('/src/OperationsStore.tsx')) return source+'\nexport { Context as TestContext };'
      if(id.endsWith('/src/CatalogStore.tsx')) return source+'\nexport { CatalogContext as TestCatalog };'
    },
    configureServer(vite) {
      vite.middlewares.use('/__qa',async (_req,res) => {
        const fixture=await readFile(new URL('./operations-fixture.mjs',import.meta.url),'utf8')
        const html=await vite.transformIndexHtml('/__qa',`<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"/></head><body><div id="root"></div><script type="module">
          ...
          const catalog={services:[{id:'s1',nome:'Corte integrado',descricao:'Corte',preco:40,duracao:30,ativo:true,cor:'#203F20'}],professionals:[{id:'p1',nome:'Araújo',ativo:true,cor:'#203F20',comissao:40,iniciais:'A',especialidades:'Cortes',telefone:''},{id:'p2',nome:'Machado',ativo:true,cor:'#203F20',comissao:0,iniciais:'M',especialidades:'Cortes',telefone:''}],loading:false,error:'',updateService:async()=>null,addService:async()=>null,updateProfessional:async()=>null,addProfessional:async()=>null,deleteProfessional:async()=>null};
          const value={data:fixture,loading:false,error:'',busy:false,refreshed:'12:00',refresh:async()=>{},run:async()=>false};
